package fr.gabrielmercier.skinchanger.manager;
...
public class SSkinChangerManager {
    ...
    public static void SChangeSkin(Player player) {
        ...
        //  Send the packets
        connection.sendPacket(new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, ((CraftPlayer)player).getHandle()));
        profile.getProperties().removeAll("textures");
        profile.getProperties().put("textures", getSkin(SMain.getInstance().getConfig().getString("mode")));
        connection.sendPacket(new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, ((CraftPlayer)player).getHandle()));
    }
